...
export function AgentLifecycleChart() {
  return (
    <section className="mt-6 border border-[#70D49D]/30 bg-[#0B1716] p-5 sm:p-7" aria-labelledby="agent-lifecycle-title" data-testid="agent-lifecycle-chart">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3 lg:-mx-2 lg:grid-cols-6" aria-label="Agent commit reveal lifecycle">
        {stages.map(({ label, detail, proof, explanation, observability, icon: Icon }, index) => (
          <Tooltip key={label}>
            <TooltipTrigger asChild>
              <button type="button" className="group relative min-h-[236px] w-full rounded-[14px] border border-white/15 bg-[#12201D] p-6 text-left transition-[transform,border-color,background-color,box-shadow] duration-200 ease-out hover:-translate-y-1 hover:border-[#F0563A]/70 hover:bg-[#172A25] hover:shadow-[0_14px_28px_rgba(0,0,0,0.24),0_0_0_1px_rgba(240,86,58,0.18)] focus-visible:-translate-y-1 focus-visible:border-[#F0563A] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F0563A]/70 motion-reduce:transform-none motion-reduce:transition-none" aria-label={`${label}: ${explanation}`}>
                <div className="flex items-center justify-between gap-2">
                  <span className="font-mono text-xs font-semibold tracking-[0.14em] text-[#F0563A]">{String(index + 1).padStart(2, "0")}</span>
                  <Icon className="h-5 w-5 text-[#70D49D] transition-[transform,color] duration-200 ease-out group-hover:scale-110 group-hover:text-[#F0563A] motion-reduce:transition-none" aria-hidden="true" />
                </div>
                <div className="mt-8 font-display text-2xl font-semibold text-[#F3EEE5] transition-[transform,color] duration-200 ease-out group-hover:-translate-y-0.5 group-hover:text-[#FFF9EF] motion-reduce:transition-none">{label}</div>
                <div className="mt-3 text-sm leading-6 text-[#CFC7BC]">{detail}</div>
                <div className="mt-8 border-t border-white/10 pt-4 font-mono text-[10px] leading-4 tracking-[0.05em] text-[#70D49D]">{proof}</div>
                <div className="mt-3 font-mono text-[9px] tracking-[0.1em] text-[#7F8F97]">FOCUS FOR DETAIL ↗</div>
