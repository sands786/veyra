...
export function MainnetEvidenceStrip({
  ...
  return (
    <section
      className="mt-8 overflow-hidden rounded-[20px] border border-[#70D49D]/35 bg-[#0B1716] p-6 sm:p-8"
    ...
    >
      ...
      <div className="grid gap-px overflow-hidden rounded-[14px] bg-white/10 sm:grid-cols-3" aria-label="Mainnet evidence facts">
        <div className="bg-[#0D201D] px-5 py-7 sm:px-6">
          <div className="font-mono text-[10px] font-semibold tracking-[0.14em] text-[#AEB8BE]">CONTRACT / MAINNET</div>
          <div className="mt-4 break-all font-mono text-lg font-semibold text-[#F3EEE5] sm:text-xl">{shortHash(contract)}</div>
        ...
        <div className="bg-[#0D201D] px-5 py-7 sm:px-6">
          ...
          <div className="mt-4 text-lg font-semibold leading-7 text-[#70D49D] sm:text-xl">{verifiedLabel}</div>
        ...
        <div className="bg-[#0D201D] px-5 py-7 sm:px-6">
          ...
          <div className="mt-4 text-lg font-semibold leading-7 text-[#F3EEE5] sm:text-xl">Public execution and state</div>
      ...
      <div className="mt-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3 lg:grid-cols-6" aria-label={`Lifecycle: ${lifecycle}`}>
          {stages.map((stage, index) => (
            <div key={`${stage}-${index}`} className="relative min-w-0">
              <span className="inline-flex min-h-16 w-full items-center justify-center rounded-[12px] border border-white/15 bg-[#14231F] px-3 text-center font-mono text-sm font-semibold tracking-[0.04em] text-[#F3EEE5] transition-[border-color,background-color,transform] duration-200 hover:-translate-y-0.5 hover:border-[#F0563A]/60 hover:bg-[#172A25] focus-within:outline-none motion-reduce:transform-none motion-reduce:transition-none">{String(index + 1).padStart(2, "0")} / {stage}</span>
      ...
      <div className="mt-7 rounded-[14px] border-l-2 border-[#F0563A] bg-[#1B1A18] px-4 py-4 sm:px-5">
        <div className="font-mono text-[10px] font-semibold tracking-[0.16em] text-[#F0563A]">PRIVACY BOUNDARY</div>
        <p className="mt-3 text-base leading-7 text-[#E2DBD1]">{privacyNote}</p>
      </div>
    </section>
  );
}
