...
export default function PrivatePrimitives() {
  ...
  async function copyLink() {
    const url = `${window.location.origin}${privateClaimDemoPath}`;
      ...
      <main className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:px-12 lg:py-16">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-[280px_1fr]">
          ...
          <section className="border border-white/10 bg-[#171815] p-6 sm:p-8" aria-live="polite"><div className="flex items-start justify-between gap-5"><div><div className="flex items-center gap-2 text-[#F0563A]"><ActiveIcon size={17} /><span className="eyebrow">{active.kicker}</span></div><h2 className="mt-5 max-w-2xl font-display text-3xl font-bold tracking-[-0.05em] sm:text-5xl">{active.title}</h2><p className="mt-5 max-w-2xl text-sm leading-6 text-[#B7B0A6]">{active.description}</p></div><Sparkles size={24} className="hidden text-[#F0563A] sm:block" /></div><div className="mt-8 grid gap-3 sm:grid-cols-3">{active.points.map((point) => <div key={point} className="border-t border-white/15 pt-3 font-mono text-[10px] leading-5 text-[#D8D0C5]">{point}</div>)}</div>
            {activeId === "links" && <div className="mt-10 border border-white/10 bg-[#20211E] p-5"><div className="flex items-center gap-2 font-mono text-[10px] tracking-[0.1em] text-[#918B81]"><Link2 size={14} /> DEMO PAYMENT REQUEST</div><div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-display text-lg">Contractor route · 2,840 USDC</div><div className="mt-1 font-mono text-[9px] text-[#918B81]">EXPIRES IN 7 DAYS · RECIPIENT SCOPED</div></div><Button onClick={() => { setLinkCreated(true); }} className="h-9 rounded-[9px] bg-[#F0563A] font-mono text-[9px] text-[#111210]">{linkCreated ? "LINK READY" : "CREATE PRIVATE LINK"}</Button></div>{linkCreated && <button onClick={() => void copyLink()} className="mt-4 flex w-full items-center gap-2 truncate border-t border-white/10 pt-3 text-left font-mono text-[10px] text-[#70D49D]"><Copy size={13} /> {window.location.origin}{privateClaimDemoPath}</button>}</div>}
            {activeId === "proofs" && <div className="mt-10 border border-white/10 bg-[#20211E] p-5"><div className="flex items-center justify-between"><div className="flex items-center gap-2 font-mono text-[10px] tracking-[0.1em] text-[#918B81]"><Fingerprint size={14} /> SELECTIVE DISCLOSURE PREVIEW</div><span className="font-mono text-[9px] text-[#70D49D]">PRIVATE BY DEFAULT</span></div><div className="mt-5 grid gap-4 sm:grid-cols-3"><div><div className="font-mono text-[9px] text-[#918B81]">RECIPIENTS</div><div className="mt-1 font-display text-2xl">6</div></div><div><div className="font-mono text-[9px] text-[#918B81]">ROSTER</div><div className="mt-1 font-display text-2xl text-[#70D49D]">HIDDEN</div></div><div><div className="font-mono text-[9px] text-[#918B81]">STATUS</div><div className="mt-1 font-display text-2xl">SETTLED</div></div></div><Button onClick={() => setProofReady(true)} className="mt-6 h-9 rounded-[9px] border border-white/15 bg-transparent font-mono text-[9px] text-[#F3EEE5]">{proofReady ? "PROOF SUMMARY READY" : "GENERATE PROOF SUMMARY"}</Button>{proofReady && <div className="mt-4 border-t border-white/10 pt-3 font-mono text-[9px] leading-5 text-[#918B81]">Public signal: route settled · 6 recipients · commitment verified · individual amounts withheld.</div>}</div>}
            {activeId === "milestones" && <div className="mt-10 border border-white/10 bg-[#20211E] p-5"><div className="flex items-center justify-between"><div className="font-mono text-[10px] tracking-[0.1em] text-[#918B81]">MILESTONE RELEASE LANE</div><span className="font-mono text-[9px] text-[#F0563A]">EVIDENCE FIRST</span></div><div className="mt-5 grid gap-2 sm:grid-cols-4">{milestoneSteps.map((step, index) => <div key={step} className={`border-t-2 pt-3 font-mono text-[9px] ${index < 2 || milestoneReady ? "border-[#70D49D] text-[#F3EEE5]" : "border-white/15 text-[#918B81]"}`}><div>{String(index + 1).padStart(2, "0")}</div><div className="mt-1">{step}</div></div>)}</div><Button onClick={() => setMilestoneReady(true)} className="mt-7 h-9 rounded-[9px] bg-[#F3EEE5] font-mono text-[9px] text-[#111210]">{milestoneReady ? "RELEASE READY / 2 APPROVERS" : "MARK EVIDENCE READY"}</Button></div>}
          </section>
        </div>
        <div className="mt-10 flex items-center gap-2 font-mono text-[9px] tracking-[0.1em] text-[#918B81]"><Check size={13} className="text-[#70D49D]" /> DEMO STATES ARE LOCAL SIMULATIONS. LIVE TESTNET OR MAINNET PROOF STILL REQUIRES WALLET APPROVAL.</div>
      </main>
    </div>
  );
}
