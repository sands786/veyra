...
const chapters = documentationChapters.map((id) => ({
  id,
  label: id === "why" ? "Why Veyra" : id === "product" ? "Product map" : id === "privacy" ? "Privacy model" : id === "starknet" ? "Starknet flow" : id === "demo" ? "Demo Mode" : "Overview",
}));
    ...
    <div className="min-h-screen bg-[#111210] text-[#F2EEE7]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[204px] border-r border-white/10 bg-[#151614] lg:block">
        <div className="flex h-full flex-col px-5 py-6">
          <Link href="/" className="mb-12 no-underline"><VeyraBrand compact /></Link>
      ...
      <main className="lg:pl-[204px]">
        <header className="sticky top-0 z-20 flex items-center justify-between border-b border-white/10 bg-[#111210]/90 px-5 py-4 backdrop-blur lg:px-10">
          <div className="flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.18em] text-[#AEB8BE]"><span className="h-2 w-2 rounded-full bg-[#6DE3A1]" />Veyra / Product documentation</div>
        ...
        <div className="mx-auto max-w-[1180px] px-5 pb-24 pt-10 lg:px-10 lg:pt-16">
          <section id="overview" className="scroll-mt-24">
            ...
            <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
              <div><h1 className="max-w-4xl font-sans text-5xl font-semibold leading-[0.9] tracking-[-0.07em] sm:text-7xl lg:text-[7.2rem]">Private money.<br /><span className="text-[#F0563A]">Clear intent.</span></h1><p className="mt-7 max-w-2xl text-base leading-7 text-[#AAA49A] sm:text-lg">Veyra is a privacy-first operating system for payroll, treasury governance, private claims, and Starknet project launches. This guide explains what the product does, why it exists, and where the Demo Mode boundary ends.</p></div>
              <div className="border border-[#F0563A]/40 bg-[#1B2930] p-5"><div className="mb-5 flex items-center justify-between gap-3 font-mono text-[10px] uppercase tracking-[0.18em] text-[#F0563A]"><span className="flex items-center gap-2"><Sparkles size={14} />Veyra / cinematic product film</span><span className="border border-[#6DE3A1]/35 px-2 py-1 text-[8px] text-[#6DE3A1]">03:10</span></div><div className="aspect-video overflow-hidden border border-white/10 bg-black"><video className="h-full w-full object-cover" controls playsInline preload="metadata"><source src={teaserVideo} type="video/mp4" />Your browser does not support the Veyra product film.</video></div><div className="mt-4 border-t border-white/10 pt-4"><p className="font-sans text-base text-[#F2EEE7]">A stabilized Veyra product film with deliberate title cards, one-direction product scrolls, and an explicit receipt boundary.</p><p className="mt-2 font-mono text-[9px] uppercase tracking-[0.14em] text-[#AEB8BE]">STABLE EDITORIAL MOTION · CONTROLLED PRODUCT SCROLLS · RECEIPT BOUNDARY EXPLICIT</p></div></div>
            </div>
          </section>

          <section id="why" className="scroll-mt-24 border-t border-white/10 pt-20 mt-24">
            <SectionLabel>02 / Why Veyra</SectionLabel>
            <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]"><h2 className="font-sans text-4xl font-semibold tracking-[-0.06em] sm:text-6xl">The gap is not another dashboard.</h2><div className="space-y-5 text-[#AAA49A] leading-7"><p>Teams need payment coordination, but the default workflow leaks context: who was paid, how much, which contributors belong to a project, and how treasury decisions were made. Public verification is valuable; public exposure of the full roster is not.</p><p>Veyra separates those concerns. Operators get a durable workspace for intent, approvals, schedules, and receipts. Public observers get aggregate proof surfaces. Claimants receive private redemption paths. The product is designed around the privacy boundary instead of adding privacy as a late toggle.</p><div className="grid gap-3 pt-3 sm:grid-cols-3"><div className="border border-white/10 p-4"><div className="font-mono text-2xl text-[#F0563A]">01</div><p className="mt-3 text-sm text-[#D5CEC4]">Private roster</p><p className="mt-1 text-xs text-[#77736C]">Identity and amount context stays scoped.</p></div><div className="border border-white/10 p-4"><div className="font-mono text-2xl text-[#F0563A]">02</div><p className="mt-3 text-sm text-[#D5CEC4]">Governed intent</p><p className="mt-1 text-xs text-[#77736C]">Policies and approvals shape execution.</p></div><div className="border border-white/10 p-4"><div className="font-mono text-2xl text-[#F0563A]">03</div><p className="mt-3 text-sm text-[#D5CEC4]">Public proof</p><p className="mt-1 text-xs text-[#77736C]">Verification without the roster dump.</p></div></div></div></div>
          </section>

          <section id="product" className="scroll-mt-24 border-t border-white/10 pt-20 mt-24"><SectionLabel>03 / Product map</SectionLabel><div className="grid gap-4 sm:grid-cols-2">{productSurfaces.map(({ icon: Icon, index, title, copy }) => <article key={title} className="group border border-white/10 bg-[#151D21] p-6 transition hover:border-[#F0563A]/50"><div className="flex items-start justify-between"><div className="grid h-10 w-10 place-items-center border border-[#F0563A]/40 text-[#F0563A]"><Icon size={18} /></div><span className="font-mono text-[10px] text-[#6E6A63]">{index}</span></div><h3 className="mt-8 font-sans text-2xl font-medium tracking-[-0.04em]">{title}</h3><p className="mt-3 text-sm leading-6 text-[#918C83]">{copy}</p></article>)}</div></section>

          <section id="privacy" className="scroll-mt-24 border-t border-white/10 pt-20 mt-24"><SectionLabel>04 / Privacy model</SectionLabel><div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start"><div><h2 className="font-sans text-4xl font-semibold tracking-[-0.06em] sm:text-6xl">Reveal the proof.<br /><span className="text-[#F0563A]">Not the roster.</span></h2><p className="mt-5 max-w-xl leading-7 text-[#AAA49A]">The operating model is intentionally asymmetric: private inputs and operational context on one side, a small public proof surface on the other. This is the core product decision behind shielded allocations, recipient claim links, and aggregate health cards.</p></div><div className="border border-white/10 bg-[#151D21] p-6"><div className="mb-6 flex items-center justify-between"><span className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#AEB8BE]">Interactive privacy boundary</span><button onClick={() => setRevealPrivate((value) => !value)} className="font-mono text-[10px] uppercase tracking-[0.12em] text-[#F0563A] underline">{revealPrivate ? "Hide context" : "Preview private context"}</button></div><div className="grid gap-3 sm:grid-cols-3">{steps.map(({ label, detail, icon: Icon }, index) => <button key={label} onClick={() => setSimulationStep(index)} className={`border p-4 text-left transition ${simulationStep === index ? "border-[#F0563A] bg-[#1B2930]" : "border-white/10 hover:border-white/25"}`}><Icon size={17} className={simulationStep === index ? "text-[#F0563A]" : "text-[#AEB8BE]"} /><div className="mt-6 font-mono text-[10px] uppercase tracking-[0.13em] text-[#D5CEC4]">{label}</div><div className="mt-2 text-xs leading-5 text-[#77736C]">{detail}</div></button>)}</div><div className="mt-5 border-t border-white/10 pt-5 font-mono text-[10px] uppercase tracking-[0.12em] text-[#6DE3A1]">{simulationStep === 0 ? "Workspace only / claimant context stays private" : simulationStep === 1 ? "Shielded intent / execution boundary enforced" : "Public proof / aggregate state only"}</div>{revealPrivate && <div className="mt-4 border border-[#F0563A]/30 bg-[#1B2930] p-4 text-xs text-[#D5CEC4]">Example private context: <span className="text-[#F0563A]">Alice · 0xDEMO…A1 · 1,200 USDC</span>. This is a local visual simulation, not a real wallet lookup or transaction.</div>}</div></div></section>

          <section id="videos" className="scroll-mt-24 border-t border-white/10 pt-20 mt-24"><SectionLabel>05 / Function videos</SectionLabel><div className="flex flex-col gap-5"><div className="max-w-3xl"><h2 className="font-sans text-4xl font-semibold tracking-[-0.06em] sm:text-6xl">See every function.<br /><span className="text-[#F0563A]">One video at a time.</span></h2><p className="mt-5 leading-7 text-[#AAA49A]">Each standalone walkthrough isolates one product responsibility. Watch the interaction, read the explanation, then compare the simulated boundary with the production execution requirement.</p></div><div className="grid gap-5 lg:grid-cols-2">{documentationVideoGuides.map((guide, index) => <article key={guide.id} className="overflow-hidden border border-white/10 bg-[#151D21]"><div className="aspect-video bg-black"><video className="h-full w-full object-cover" controls playsInline preload="metadata"><source src={guide.asset} type="video/mp4" />Your browser does not support this video.</video></div><div className="p-5"><div className="flex items-center justify-between gap-4"><div className="font-mono text-[10px] uppercase tracking-[0.16em] text-[#F0563A]">0{index + 1} / {guide.title}</div><span className="font-mono text-[9px] text-[#6DE3A1]">{guide.duration}</span></div><p className="mt-4 text-sm leading-6 text-[#D5CEC4]">{guide.purpose}</p><div className="mt-4 border-l-2 border-[#F0563A]/50 pl-3 text-xs leading-5 text-[#AEB8BE]"><span className="font-mono uppercase tracking-[0.12em] text-[#F0563A]">Boundary</span><br />{guide.boundary}</div></div></article>)}</div></div></section>

          <section id="starknet" className="scroll-mt-24 border-t border-white/10 pt-20 mt-24"><SectionLabel>06 / Starknet flow</SectionLabel><div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]"><div className="border border-white/10 bg-[#151D21] p-6"><div className="flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-[0.14em] text-[#AEB8BE]"><span className="border border-[#F0563A]/40 px-3 py-2 text-[#F0563A]">Operator intent</span><ChevronRight size={14} /><span className="border border-white/10 px-3 py-2">STRK20 shield</span><ChevronRight size={14} /><span className="border border-white/10 px-3 py-2">Wallet signature</span><ChevronRight size={14} /><span className="border border-[#6DE3A1]/40 px-3 py-2 text-[#6DE3A1]">Receipt / proof</span></div><div className="mt-10 grid gap-4 sm:grid-cols-2"><div className="border border-white/10 p-5"><WalletCards className="text-[#F0563A]" size={20} /><h3 className="mt-5 text-lg">Account-aware execution</h3><p className="mt-2 text-sm leading-6 text-[#918C83]">Veyra keeps the wallet and provider boundary explicit. Demo Mode never creates a private key or pretends a simulated receipt is mainnet evidence.</p></div><div className="border border-white/10 p-5"><Shield className="text-[#6DE3A1]" size={20} /><h3 className="mt-5 text-lg">Auditable privacy</h3><p className="mt-2 text-sm leading-6 text-[#918C83]">The product can show workflow health, approvals, and aggregate proofs without making every participant a public data point.</p></div></div></div><div className="border border-[#F0563A]/30 bg-[#1B2930] p-6"><div className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#F0563A]">Execution boundary</div><p className="mt-5 text-2xl leading-tight text-[#F2EEE7]">Production actions require authenticated workspace access and a connected wallet.</p><p className="mt-5 text-sm leading-6 text-[#AAA49A]">The Documentation page explains the real product. Demo Mode is the safe place to explore the interaction model before supplying real credentials or signing anything.</p><Link href="/demo" className="mt-6 inline-flex items-center gap-2 border border-[#F0563A] px-4 py-3 font-mono text-[10px] uppercase tracking-[0.13em] text-[#F0563A] hover:bg-[#F0563A] hover:text-[#111210]">Explore Demo Mode <ArrowUpRight size={14} /></Link></div></div></section>

          <section id="demo" className="scroll-mt-24 border-t border-white/10 pt-20 mt-24"><SectionLabel>07 / Demo Mode</SectionLabel><div className="grid gap-8 lg:grid-cols-[0.75fr_1.25fr]"><div><h2 className="font-sans text-4xl font-semibold tracking-[-0.06em] sm:text-6xl">Touch every<br /><span className="text-[#F0563A]">privacy surface.</span></h2><p className="mt-5 leading-7 text-[#AAA49A]">The guided tour uses deterministic local state to demonstrate payroll, operations, treasury, claims, Launchpad, and proof. Each action has visible success, failure, retry, and reset behavior so the product story can be understood without a wallet.</p></div><div className="border border-white/10 bg-[#151D21] p-6"><div className="flex items-center justify-between"><div className="font-mono text-[10px] uppercase tracking-[0.17em] text-[#AEB8BE]">Simulation checklist</div><span className="font-mono text-[10px] text-[#6DE3A1]">LOCAL / REVERSIBLE</span></div><div className="mt-6 space-y-3">{["Build a private payroll route", "Run treasury policy and governance", "Redeem a private claim", "Advance a Launchpad milestone", "Publish an aggregate proof"].map((item, index) => <div key={item} className="flex items-center gap-3 border-b border-white/10 pb-3 text-sm text-[#D5CEC4]"><div className="grid h-6 w-6 place-items-center rounded-full border border-[#6DE3A1]/50 text-[#6DE3A1]"><Check size={13} /></div><span>{item}</span><span className="ml-auto font-mono text-[9px] text-[#6E6A63]">0{index + 1}</span></div>)}</div><Link href="/demo" className="mt-7 inline-flex w-full items-center justify-center gap-2 bg-[#F0563A] px-4 py-3 font-mono text-[10px] uppercase tracking-[0.14em] text-[#111210] hover:bg-[#ff6b4e]">Open the full guided tour <PlayCircle size={15} /></Link></div></div></section>

          <footer className="mt-24 border-t border-white/10 pt-8"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div><div className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#F0563A]">Veyra / Documentation</div><p className="mt-2 text-sm text-[#77736C]">Private payment infrastructure for teams that need proof without exposure.</p></div><div className="flex gap-3"><Link href="/" className="border border-white/15 px-4 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-[#D5CEC4]">Workspace</Link><Link href="/launchpad" className="border border-white/15 px-4 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-[#D5CEC4]">Launchpad</Link></div></div></footer>
        </div>
      </main>
    </div>
  );
