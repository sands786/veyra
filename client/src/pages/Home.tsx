...
export default function Home() {
  ...
  const updateRecipientMutation = trpc.recipients.update.useMutation({ onSuccess: async () => { await utils.recipients.list.invalidate(); await utils.workspace.overview.invalidate(); setEditingRecipientId(null); setRecipientName(""); setRecipientWallet(""); toast("Recipient updated."); } });
  const transitionRouteMutation = trpc.routes.transition.useMutation({ onSuccess: async () => { await utils.routes.list.invalidate(); await utils.workspace.overview.invalidate(); toast("Route status updated."); } });
  ...
  const displayActivity = isAuthenticated
    ...
    : activity;
  const workspaceError = overviewQuery.error ?? recipientsQuery.error ?? auditQuery.error;
