    ...
    <div className="min-h-screen overflow-hidden bg-[#111210] text-[#F3EEE5] selection:bg-[#F0563A] selection:text-[#111210]">
      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        <main className="min-w-0 flex-1">
          <header className="relative flex items-center justify-between border-b border-white/10 px-5 py-4 sm:px-8 lg:px-8">
            ...
            <div className="flex items-center gap-3">
              {isAuthenticated ? <label className="hidden items-center gap-2 font-mono text-[10px] tracking-[0.12em] text-[#918B81] sm:flex"><select aria-label="Active workspace" value={activeWorkspaceId ?? ""} onChange={(event) => switchWorkspace(event.target.value)} className="max-w-[150px] bg-transparent text-[#F3EEE5] outline-none"><option value="" disabled>{user?.name ?? "ACTIVE"}</option>{(workspaceListQuery.data ?? []).map((membership) => <option key={membership.workspace.id} value={membership.workspace.id}>{membership.workspace.name}</option>)}</select></label> : null}
