...
export default function Home() {
  ...
  const createRecipientMutation = trpc.recipients.create.useMutation({ onSuccess: async () => { await utils.recipients.list.invalidate(); await utils.workspace.overview.invalidate(); setRecipientName(""); setRecipientWallet(""); toast("Recipient added to the private roster."); } });
  const createRouteMutation = trpc.routes.create.useMutation({ onSuccess: async () => { await utils.routes.list.invalidate(); await utils.workspace.overview.invalidate(); } });
  const recordTransactionMutation = trpc.transactions.recordSubmission.useMutation({ onSuccess: async () => { await utils.routes.list.invalidate(); await utils.workspace.overview.invalidate(); } });
  const archiveRecipientMutation = trpc.recipients.archive.useMutation({ onSuccess: async () => { await utils.recipients.list.invalidate(); await utils.workspace.overview.invalidate(); toast("Recipient archived."); } });
  const restoreRecipientMutation = trpc.recipients.restore.useMutation({ onSuccess: async () => { await utils.recipients.list.invalidate(); await utils.workspace.overview.invalidate(); toast("Recipient restored."); } });
  const displayActivity = isAuthenticated
    ? liveRoutes.map((route) => ({ id: `VP-${String(route.id).padStart(3, "0")}`, title: route.name, detail: `${route.token} · ${route.totalAmount}`, state: route.status.toUpperCase(), time: new Date(route.createdAt).toLocaleDateString() }))
    : activity;
  const workspaceError = overviewQuery.error ?? recipientsQuery.error;
  ...
  async function advanceRoute() {
    ...
    if (stage === 1) {
      try {
        savedRoute = await createRouteMutation.mutateAsync({ name: routeName.trim() || "Untitled private route", token: tokenSymbol, totalAmount: amount.replace(/,/g, ""), recipientAmounts: activeRecipientIds.map((recipientId) => ({ recipientId, amount: amount.replace(/,/g, "") })) });
      } catch (error) {
    ...
    <div className="min-h-screen overflow-hidden bg-[#111210] text-[#F3EEE5] selection:bg-[#F0563A] selection:text-[#111210]">
      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        <main className="min-w-0 flex-1">
          ...
          {workspaceError && isAuthenticated && <div className="flex items-center justify-between gap-4 border-b border-[#F0563A]/30 bg-[#F0563A]/10 px-5 py-3 font-mono text-[10px] tracking-[0.08em] text-[#FFB1A3] sm:px-8 lg:px-12"><span>WORKSPACE DATA COULD NOT LOAD / {workspaceError.message.slice(0, 100)}</span><button onClick={() => { void overviewQuery.refetch(); void recipientsQuery.refetch(); }} className="shrink-0 rounded-full border border-[#F0563A]/40 px-3 py-1 text-[#F3EEE5] hover:bg-[#F0563A] hover:text-[#111210]">RETRY</button></div>}
