...
const stages = [
  ...
  { label: "CONFIRMED", note: "A wallet-signed transaction and receipt close the loop." },
];

const activity = [
  { id: "DEMO-019", routeId: undefined, title: "March contractor run", detail: "DEMO · 6 recipients · USDC", state: "DEMO / PROOF READY", time: "8 min ago" },
  { id: "DEMO-018", routeId: undefined, title: "Design retainer split", detail: "DEMO · 3 recipients · STRK", state: "DEMO / SHIELDED", time: "Yesterday" },
  { id: "DEMO-017", routeId: undefined, title: "Ops reimbursement", detail: "DEMO · 2 recipients · ETH", state: "DEMO / CONFIRMED", time: "Apr 04" },
];
...
export default function Home() {
  ...
  const createProofMutation = trpc.proofs.create.useMutation({ onError: (error, input) => { setMutationError(error.message); setRetryAction(() => () => createProofMutation.mutate(input)); }, onSuccess: (result) => { setProofSlug(result.slug); toast("Receipt-backed proof link created.", { description: "Available only after the route has a confirmed Starknet receipt." }); } });
  ...
  const claimCreateMutation = trpc.claims.create.useMutation({ onError: (error, input) => { setMutationError(error.message); setRetryAction(() => () => claimCreateMutation.mutate(input)); }, onSuccess: (result) => { setClaimToken(result.token); toast("Unsigned private claim link created.", { description: "This creates a persisted request; it does not sign or move funds." }); } });
        ...
        <main className="min-w-0 flex-1">
          ...
          <section className="relative isolate overflow-hidden border-b border-white/10 px-5 py-11 sm:px-8 lg:px-8 lg:py-14">
            ...
            <div className="max-w-[900px]">
              ...
              <div className="mt-9 flex flex-wrap items-center gap-3 sm:gap-5"><Button onClick={() => document.getElementById("route-builder")?.scrollIntoView({ behavior: "smooth" })} className="group h-12 rounded-full bg-[#F3EEE5] px-6 font-mono text-[10px] tracking-[0.13em] text-[#111210] hover:bg-white">CREATE PRIVATE ROUTE <ArrowUpRight className="ml-2 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" size={15} /></Button><span className="font-mono text-[10px] leading-5 tracking-[0.1em] text-[#AEB8BE]">PUBLICLY VERIFIABLE<br />AFTER RECEIPT CONFIRMATION</span></div>
            </div>
          </section>

          <section className="border-b border-white/10 bg-[#0D1010] px-5 py-16 sm:px-8 lg:px-8 lg:py-24" aria-labelledby="veyra-teaser-title">
            <div className="mx-auto grid max-w-[1180px] gap-14 lg:grid-cols-[0.72fr_1.28fr] lg:items-end">
              <div className="max-w-[430px]"><div className="eyebrow">02 / VIEWING ROOM</div><h2 id="veyra-teaser-title" className="mt-5 font-display text-[clamp(3rem,5.3vw,5.4rem)] font-semibold leading-[0.88] tracking-[-0.075em] text-[#F3EEE5]">See the boundary.<br /><span className="text-[#F0563A]">Feel the system.</span></h2><p className="mt-7 max-w-[390px] text-[15px] leading-7 text-[#BDB5A9]">A thirty-second institutional cut for Veyra’s private-payroll operating model: governed intent, sealed allocations, private markets, and receipt-backed proof.</p><div className="mt-9 border-y border-white/10 py-4"><div className="grid grid-cols-3 gap-3 font-mono text-[8px] uppercase tracking-[0.12em]"><div><div className="text-[#6E6A63]">Format</div><div className="mt-2 text-[#F3EEE5]">30 sec</div></div><div><div className="text-[#6E6A63]">Audio</div><div className="mt-2 text-[#F0563A]">Original score</div></div><div><div className="text-[#6E6A63]">Motion</div><div className="mt-2 text-[#6DE3A1]">Stable master</div></div></div></div></div>
              <div className="border border-[#F0563A]/45 bg-[#0A0C0C] p-3 sm:p-4 shadow-[0_28px_80px_rgba(0,0,0,0.42)]"><div className="flex items-center justify-between border-b border-white/10 pb-3 font-mono text-[9px] uppercase tracking-[0.15em]"><span className="text-[#F0563A]">Film 01 / Product thesis</span><span className="border border-[#6DE3A1]/35 px-2 py-1 text-[8px] text-[#6DE3A1]">Stable cut</span></div><div className="mt-3 border border-white/15 bg-black p-1"><video className="aspect-video w-full bg-black object-cover" controls playsInline preload="metadata" aria-label="Thirty-second Veyra teaser with voiceover and original score"><source src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663488625248/UWySWLIQYVhglQGv.mp4" type="video/mp4" />Your browser does not support the Veyra teaser.</video></div><div className="mt-4 grid gap-3 border-t border-white/10 pt-4 sm:grid-cols-[1fr_auto] sm:items-end"><p className="max-w-[420px] text-[12px] leading-5 text-[#D5CEC4]">A private coordination system should make its execution boundary legible: prepare, authorize, confirm, and prove.</p><div className="font-mono text-[8px] uppercase tracking-[0.12em] text-[#AEB8BE]">Wallet → Receipt → Proof</div></div></div>
            </div>
          </section>
