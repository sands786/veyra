...
export const routeRecipients = mysqlTable(
  ...
  table => ({
    routeRecipientUnique: uniqueIndex(
      "routeRecipients_route_recipient_unique"
    ).on(table.routeId, table.recipientId),
  })
);

export const payrollSchedules = mysqlTable(
  "payrollSchedules",
  {
    id: int("id").autoincrement().primaryKey(),
    workspaceId: int("workspaceId").notNull(),
  ...
  table => ({
    taskUidIdx: index("payrollSchedules_task_uid_idx").on(
...
export const claimLinks = mysqlTable("claimLinks", {
  ...
  expiresAt: timestamp("expiresAt").notNull(),
  status: mysqlEnum("status", ["pending", "claimed", "revoked"])
    .notNull()
    .default("pending"),
...
export const launchpadProjects = mysqlTable("launchpadProjects", {
  ...
  privacyMode: mysqlEnum("privacyMode", ["shielded", "public"])
    .notNull()
    .default("shielded"),
  status: mysqlEnum("status", ["draft", "live", "funded", "closed"])
    .notNull()
    .default("draft"),
...
export const launchpadMilestones = mysqlTable("launchpadMilestones", {
  ...
  approvalThreshold: int("approvalThreshold").notNull().default(1),
  status: mysqlEnum("status", ["planned", "ready", "released", "blocked"])
    .notNull()
    .default("planned"),
  proofReference: varchar("proofReference", { length: 255 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
...
export const launchpadProjectOps = mysqlTable(
  "launchpadProjectOps",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    roundType: mysqlEnum("roundType", [
      ...
      .notNull()
      .default("community"),
    stage: mysqlEnum("stage", ["planning", "review", "live", "closeout"])
      .notNull()
      .default("planning"),
    ...
    readinessOverride: mysqlEnum("readinessOverride", [
      "none",
      "blocked",
      "ready",
    ])
...
export const launchpadReleaseRequests = mysqlTable(
  "launchpadReleaseRequests",
  {
    ...
    status: mysqlEnum("status", ["pending", "approved", "rejected", "settled"])
      .notNull()
      .default("pending"),
  ...
  table => ({
    projectIdx: index("launchpadReleaseRequests_project_idx").on(
      table.projectId
    ),
    milestoneIdx: index("launchpadReleaseRequests_milestone_idx").on(
      table.milestoneId
    ),
...
export const launchpadAllocations = mysqlTable("launchpadAllocations", {
  ...
  status: mysqlEnum("status", ["reserved", "claimed", "revoked"])
    .notNull()
    .default("reserved"),
  claimedWalletAddress: varchar("claimedWalletAddress", { length: 80 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
...
export const privateMarketBids = mysqlTable(
  "privateMarketBids",
  {
    id: int("id").autoincrement().primaryKey(),
    marketId: int("marketId").notNull(),
    bidderUserId: int("bidderUserId").notNull(),
    commitmentHash: varchar("commitmentHash", { length: 255 })
      .notNull()
      .unique(),
  ...
  table => ({
    marketIdx: index("privateMarketBids_market_idx").on(table.marketId),
    bidderIdx: index("privateMarketBids_bidder_idx").on(table.bidderUserId),
  })
);

export const privateMarketQuotes = mysqlTable(
  "privateMarketQuotes",
  {
    id: int("id").autoincrement().primaryKey(),
    marketId: int("marketId").notNull(),
    ...
    status: mysqlEnum("status", ["open", "accepted", "expired", "rejected"])
      .notNull()
      .default("open"),
    expiresAt: timestamp("expiresAt").notNull(),
    createdByUserId: int("createdByUserId").notNull(),
  ...
  table => ({
    marketIdx: index("privateMarketQuotes_market_idx").on(table.marketId),
    statusIdx: index("privateMarketQuotes_status_idx").on(table.status),
  })
);

export const privateMarketRiskPolicies = mysqlTable(
  ...
  table => ({
    workspaceIdx: index("privateMarketRiskPolicies_workspace_idx").on(
      table.workspaceId
    ),
    marketIdx: index("privateMarketRiskPolicies_market_idx").on(table.marketId),
  })
);

export const privateMarketAlerts = mysqlTable(
  "privateMarketAlerts",
  {
    id: int("id").autoincrement().primaryKey(),
    workspaceId: int("workspaceId").notNull(),
  ...
  table => ({
    workspaceIdx: index("privateMarketAlerts_workspace_idx").on(
      table.workspaceId
    ),
    statusIdx: index("privateMarketAlerts_status_idx").on(table.status),
    marketIdx: index("privateMarketAlerts_market_idx").on(table.marketId),
  })
);
...
export const blockchainTransactions = mysqlTable("blockchainTransactions", {
  ...
  transactionHash: varchar("transactionHash", { length: 100 })
    .notNull()
    .unique(),
  status: mysqlEnum("status", ["submitted", "confirmed", "reverted", "unknown"])
    .notNull()
    .default("submitted"),
...
export type TreasuryBalanceSnapshot =
  typeof treasuryBalanceSnapshots.$inferSelect;
